      ...
      <div class="flex bg-white flex-col items-center justify-center">
        <div class="w-full relative p-2 overflow-x-auto shadow-md sm:rounded-lg xs:p-5">
          ...
          <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-100">
              <tr>
                ...
                <th scope="col" class="px-3 py-2 w-40">Title</th>
                <th scope="col" class="px-4 py-2 w-[60%]">Content</th>
                <th scope="col" class="px-3 py-2 w-6">Created At</th>
                <th scope="col" class="px-4 py-2 w-6">Action</th>
              </tr>
            </thead>

            <tbody v-for="news in paginatedData" :key="news.id">
              <tr v-if="news" class="bg-white border-b hover:bg-gray-50">
                <td @click.prevent="countandtrash(news.id)" class="p-3 text-center">
                  <div class="border-2 border-gray-300 rounded w-5 h-5 mx-auto">
                    <p v-if="dletedId.includes(news.id)">
                      <i class="material-icons text-green-600 text-lg">done</i>
                    </p>
                  </div>
                </td>
                <td class="px-3 py-2 text-gray-900 font-semibold max-w-[140px]">
                  <span class="block truncate text-sm leading-tight" :title="news.title">
                    {{ news.title }}
                  </span>
                </td>
                ...
                <td class="px-4 py-2">
                  ...
                  >
                    <i
                      class="material-icons text-lg bg-gray-200 p-2 rounded-full hover:bg-gray-300"
                    >
                      edit
                    </i>
                  </router-link>
          ...
          <div v-if="perPage > 4" class="w-full justify-between flex flex-row pt-4">
            ...
            <div>
              <span class="p-2 bg-gray-200 text-lg font-bold rounded">{{
                currentPage
              }}</span>
            </div>
            <div>
