...
const totalPages = computed(() =>
    Math.ceil(sliders.value.length / perPage.value)
);
...
const saveChanges = async () => {
    ...
    try {
        ...
        await axios.post(
            `./api/update/sliders/${selectedSlider.value.id}`,
            formData,
            {}
        );
...
<template>
    <div class="container mx-auto p-4">
        ...
        <div class="flex justify-end mb-4 mt-3">
            <router-link
                to="/slidercreate"
                class="text-white bg-sky-700 hover:bg-sky-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
            >
                Add Slider
            </router-link>
        </div>
        ...
        <div
            v-if="showModal"
            class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 mb-10"
        >
            <div class="bg-white rounded-lg p-6 w-full max-w-lg">
                ...
                <div v-if="previewImage || selectedSlider.image" class="mt-4">
                    <img
                        :src="
                            previewImage || `/storage/${selectedSlider.image}`
                        "
