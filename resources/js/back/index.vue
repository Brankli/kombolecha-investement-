...
onMounted(async () => {
    ...
    await axios
        .get("./api/getall/question")
        .then((res) => {
            allquestions.value = res.data;
        })
        .catch((err) => console.log(err));
    await axios
        .get("./api/count/question")
        .then((res) => {
...
function viewquestion(id) {
    ...
    const newid = id;
    readQuestion.value = allquestions.value.find(
        (question) => question.id === newid
    );
}
...
<template>
    <div class="bg-gray-100 mb-24">
        ...
        <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:m-8">
            ...
            >
                <div>
                    <h1 class="text-xl text-gray-400 font-normal p-4 pb-2">
                        Total Visitor
                    </h1>
                    <h1 class="text-3xl text-sky-800 pl-4 pb-4">
                        {{ allvisitor[0] }}
                    </h1>
                </div>
                <i
                    class="fas fa-users text-sky-700 text-2xl bg-sky-200 p-2 w-fit h-fit my-auto mx-auto rounded-md"
                ></i>
            </div>
                    ...
                    >
                        ...
                        >
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3">answered</th>
                            </tr>
                        </thead>
                        <tbody
                            v-for="qestion in paginatedData"
                            :key="qestion.id"
                        >
                            <tr class="bg-white border-b hover:bg-gray-50">
                                <td class="w-4 p-4">
                                    <i
                                        @click="viewquestion(qestion.id)"
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <router-link
                                        :to="{
                                            name: 'answer',
                                            params: { id: qestion.id },
                                        }"
                                    >
                                        <i
                                            v-if="qestion.is_answered == 'no'"
                                            class="material-icons text-green-700"
                                            >unsubscribe</i
                                        ></router-link
                                    >
