    ...
    <div class="font-roboto mb-16 w-[90%] mx-auto">
        ...
        <div class="space-y-16">
            <div v-for="(category, index) in serviceCategories" :key="category.key"
                ...
                <!-- Image -->
                <img v-if="(index + 1) % 2 === 0" :src="category.image" alt="Office"
                ...
                <div class="flex-1 bg-white rounded-2xl shadow p-6 md:p-8">
                    ...
                    <ul class="space-y-3">
                        <li v-for="service in services.filter(s => s.department === category.key)"
                            :key="`${category.key}-${service.id}`" class="flex items-start gap-3">
                            <i class="material-icons text-sky-500 text-xl">done</i>
